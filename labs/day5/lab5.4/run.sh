...
verify() {
  ...
  require_tools
  expected="$(kubectl get configmap "$RELEASE" -n "$NAMESPACE" \
    -o jsonpath='{.data.index\.html}')"
  kubectl delete pod lab5-4-probe -n "$NAMESPACE" \
    --ignore-not-found --wait=false >/dev/null
  actual="$(kubectl run lab5-4-probe -n "$NAMESPACE" --rm --attach --restart=Never \
    --image=busybox:1.37 --command -- \
    wget -qO- -T 5 "http://${RELEASE}:8080/")"
  [[ "$actual" == "$expected" ]] || {
    printf 'Expected chart message %s, received %s.\n' "$expected" "$actual" >&2
    return 1
  }
  printf 'Verified Helm-managed Service response: %s\n' "$actual"
}
